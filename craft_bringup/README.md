...
1. Start robot's hardware and load controllers (default configuration starts mock hardware)
   ```
   ros2 launch craft_bringup craft.launch.py
   ```
...
1. If you want to test hardware with `ForwardCommandController` first load and configure it. Controller types are e.g., "position", "velocity", and depend on configured names in the [`config/craft_controllers.yaml`](config/craft_controllers.yaml):
   ```
...
3. Send command to the controller, either:
   ...
   ```
   ros2 launch craft_bringup test_forward_position_controller.launch.py
   ```
