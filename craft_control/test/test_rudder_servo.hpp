...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_RUDDER_SERVO_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_RUDDER_SERVO_HPP_
...
#include "craft_control/rudder_servo.hpp"
...
using ControllerStateMsg = craft_control::RudderServo::ControllerStateMsg;
using ControllerReferenceMsg = craft_control::RudderServo::ControllerReferenceMsg;
using ControllerModeSrvType = craft_control::RudderServo::ControllerModeSrvType;
...
class TestableRudderServo : public craft_control::RudderServo
{
  FRIEND_TEST(RudderServoTest, all_parameters_set_configure_success);
  FRIEND_TEST(RudderServoTest, activate_success);
  FRIEND_TEST(RudderServoTest, reactivate_success);
  FRIEND_TEST(RudderServoTest, test_setting_slow_mode_service);
  FRIEND_TEST(RudderServoTest, test_update_logic_fast);
  FRIEND_TEST(RudderServoTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = craft_control::RudderServo::on_configure(previous_state);
...
class RudderServoFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_rudder_servo/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_rudder_servo/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_rudder_servo")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_rudder_servo/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableRudderServo> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_RUDDER_SERVO_HPP_
